...
def resize_lora_model(lora_sd, new_rank, save_dtype, device, verbose):
  ...
  if network_alpha == new_alpha:
    print("network alpha is unchanged, return copy")
    ...
    for key, value in tqdm(lora_sd.items()):
      ...
      if (block_down_name == block_up_name) and weights_loaded:
        ...
        if device:
          ...
          lora_up_weight = lora_up_weight.to(device)
          lora_down_weight = lora_down_weight.to(device)
...
def resize(save_precision=None, new_rank=4, model=None, save_to=None, device=None, verbose=False):
  ...
  print("resizing rank...")
  state_dict, old_dim, new_alpha = resize_lora_model(lora_sd, new_rank, save_dtype, device, verbose)
...
if __name__ == '__main__':
  ...
  resize(args.save_precision, args.new_rank, args.model, args.save_to, args.device, args.verbose)
